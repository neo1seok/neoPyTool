...
def parse_description_file(filename):
	...
	try:
		...
		print
		"[*] Error while parsing descriptions file: " + filename
	f.close()
	return results


def make_listmap_from_description_listlist(listlist):
	listmap = [ {list[2]:list}for list in listlist]
	return listmap


def convert_command_description_listmap_from_description_listlist(listlist):
	listmap = [ {list[2]:list}for list in listlist]
	return listmap
...
def show_apdu_command(desc, cla, ins, p1, p2, lc, le, data, colors):
	...
	print("lc:",lc,"le:",le)  # INS : INS_NAME (INS_DESC)
...
def main():
	...
	parser.add_option('-c', '--commands', action='store_true', dest='commands_only', default=False,
					  help='specify if input file contains only APDU commands')
	parser.add_option('-r', '--responses', action='store_true', dest='responses_only', default=False,
					  help='specify if input file contains only APDU responses')

	parser.add_option('-C', '--command-descriptions', dest='command_descriptions', type='string',
					  help='specify custom command descriptions file')
	parser.add_option('-R', '--response-descriptions', dest='response_descriptions', type='string',
					  help='specify custom response descriptions file')

	parser.add_option('-T', '--colors', action='store_true', dest='colors', default=False,
					  help='show terminal output in different colors')
	...
	if commands_only and options.responses_only:
		print
		parser.usage
		"[*] Error: --commands and --responses options cannot be set at the same time"
		exit(0)
	...
	if not responses_only:
		if options.command_descriptions != None:
			...
			listmapcommand_descriptions = convert_command_description_listmap_from_description_listlist(command_descriptions)
		else:
